...
Port of the CRA `frontend/` app: same game flow and API calls, using the App Router and Tailwind CSS.
...
Start the API from `backend-ts` (`npm run dev`, default port **5001**), then:
...
From the repo root, `docker compose -f docker-compose.postgres.yml up --build` builds this app with `NEXT_PUBLIC_API_URL=http://localhost:5001` (correct when the browser and API are both on your machine). To change it, pass a build arg on the `frontend` service (see root `docker-compose.postgres.yml`).
